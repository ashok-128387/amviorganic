...
export default function LoginPage() {
  ...
  const handleSendOtp = async (e: React.FormEvent) => {
    ...
    try {
      await fetch('/api/send-otp', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ email: normalizedEmail, otp }) });
    } catch {
      // silently continue — OTP is still valid in store
    }
    setStep('otp');
    setResendTimer(60);
    setLoading(false);
  ...
  const handleOtpInput = (val: string, idx: number) => {
    ...
    if (digits.join('').length === 6) handleVerifyOtp(digits.join(''));
  };
            ...
            <>
              ...
              <form onSubmit={handleSendOtp} className="space-y-4">
                ...
                <div>
                  ...
                  <div className="flex items-center gap-2 border border-gray-200 rounded-xl px-3 py-2.5 focus-within:ring-2 focus-within:ring-green-700 transition">
            ...
            <>
              <div className="mb-6">
                ...
                <p className="text-sm text-gray-500">
                  A 6-digit code was sent to <strong className="text-gray-700">{normalizedEmail}</strong>
                </p>
              ...
              <div className="flex gap-2 justify-center mb-6">
                {otpDigits.map((d, i) => (
                  <input key={i} id={`otp-${i}`} type="text" inputMode="numeric" maxLength={1} value={d}
                    ...
                    style={{ borderColor: d ? '#1e4a2a' : '#e5e5e5', background: d ? '#f0faf2' : '#fff', color: '#1e4a2a' }}
                    autoFocus={i === 0} />
                ))}
              </div>
              ...
              <div className="text-center">
                <button onClick={handleResend} disabled={resendTimer > 0 || loading}
                  className="flex items-center gap-1.5 mx-auto text-xs font-semibold disabled:opacity-50"
                  style={{ color: resendTimer > 0 ? '#aaa' : '#1e4a2a' }}>
                  <RefreshCw size={13} />
                  {resendTimer > 0 ? `Resend in ${resendTimer}s` : 'Resend OTP'}
                </button>
              </div>
              <p className="text-center text-xs text-gray-400 mt-4">OTP valid for 10 minutes</p>
            </>
          )}
